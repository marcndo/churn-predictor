...
CATEGORICAL_COLUMNS = [
    ...
    "DeviceProtection", "TechSupport","StreammingTV","StreamingMovies"
]
...
def clean_data(df):
    ...
    df = df.drop(columns=COLUMNS_TO_DROP, error="ignore")
...
def split_data(df, test_size=0.2, random_state=42):
    ...
    print(f"Churn rate in train set: {X_train.mean():.2%}")
    print(f"Churn rate in test set: {X_test.mean():.2%}")
...
def save_processor(preprocessor, filepath="models/preprocessor.joblib"):
    ...
    os.path.makedirs(os.path.dirname(filepath), exist_ok=True)
